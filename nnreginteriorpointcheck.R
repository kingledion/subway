...
### Example I: Poisson regression

objective <- function(X, y, beta){    
 -sum(y * log(X %*% beta)) + sum(X %*% beta)   
}

gradient <- function(X, y, beta){
  linpred <- drop(X %*% beta)
  Dinv <- diag(1/linpred)
  -rowSums(t(X) %*% (Dinv * drop(y - X %*% beta)))
}
...
X <- X %*% diag(x = 1/norm)
set.seed(2057)
ytrain <- rpois(n, drop(X %*% beta))

### data example for the Huber loss

Phitrain <- as.matrix(read.table("/warehouse/teaching/ml0910/Phitrain.txt"))
ytrain <- as.numeric(read.table("/warehouse/teaching/ml0910/ytrain.txt")[,1])

###

check1 <- nnreginteriorpoint(X = X, y = y, objective = objective, gradient = gradient, Hessian = Hessian,
                               onlypositive = FALSE,
                               regamount = 0.5,
                               solutionpath = FALSE,
                               offset = TRUE,
...
check2 <- nnreginteriorpoint(X = X, y = y, objective = objective, gradient = gradient, Hessian = Hessian,
                               onlypositive = TRUE,
                               regamount = 0,
                               solutionpath = TRUE,
                               offset = TRUE,
...
### Example II: twice continuously differentiable Huber type loss

objective <- function(X, y, beta, epsilon){
   r <- drop(y - X %*% beta)
   a <- -1/(8 * epsilon^3) 
   b <- 3/(4 * epsilon)
   c <- 3/8 * epsilon
   losscontrib <- ifelse(abs(r) <= epsilon, 
     a * r^4 + b * r^2, abs(r) - c) 
   sum(losscontrib)
}

gradient <- function(X, y, beta, epsilon){
  r <- drop(y - X %*% beta)
  a <- -1/(8 * epsilon^3) 
  b <- 3/(4 * epsilon)
  c <- 3/8 * epsilon
  gradcontrib <- ifelse(abs(r) <= epsilon, 4 * a * r^3 + 2 * b * r,
                        sign(r)) 
  -t(X) %*% gradcontrib
}

Hessian <- function(X, y, beta, epsilon){
  r <- drop(y - X %*% beta)
  #cat(sum(r > epsilon), "\n")
  if(all(r > epsilon)) stop("Hessian is 0 \n") 
  a <- -1/(8 * epsilon^3) 
  b <- 3/(4 * epsilon)
  t(X) %*% diag(x = ifelse(abs(r) <= epsilon, 12 * a * r^2 + 2 * b, rep(0, length(r))),
                length(r), length(r)) %*% X
  
}  


check3pre <- nnreginteriorpoint(X = Phitrain, y = ytrain, objective = objective, gradient = gradient, Hessian = Hessian,
                               onlypositive = TRUE,
                               regamount = 0,
                               solutionpath = FALSE,
                               offset = FALSE,
                               ...
                               eps = 1e-6,
                               cholmod = FALSE,
                               functionarguments = list(epsilon = 100),
                               solutionpatharguments = list(regmax = NULL, gridlength = 100, fac = 0.001, udpatestart = TRUE, verbose = TRUE),
                               trace = TRUE)

check3 <- nnreginteriorpoint(X = Phitrain, y = ytrain, objective = objective, gradient = gradient, Hessian = Hessian,
                               onlypositive = TRUE,
                               regamount = 0,
                               solutionpath = TRUE,
                               offset = FALSE,
                               beta0 = check3pre$beta,
                               lambda0 = NULL, 
                               eps = 1e-6,
                               cholmod = FALSE,
                               functionarguments = list(epsilon = 1),
                               solutionpatharguments = list(regmax = 6, gridlength = 100, fac = 0.001, verbose = TRUE, updatestart = FALSE),
                               trace = FALSE)
...
pdf("huberpath_epsilon1.pdf")
matplot(colSums(check3$Beta), t(check3$Beta[A,]), col = "black", lty = 1, type = "l", xlab = expression(l[1]~~norm), ylab = expression(beta),
        main = expression(epsilon == 1), cex.main = 1.5, cex.lab = 1.6,
        cex.axis = 1.6)

matlines(colSums(check3$Beta), t(check3$Beta[setdiff(1:50,A),]), col = "grey", lty = 1, type = "l", xlab = expression(l[1]~~norm), ylab = expression(beta))

dev.off()

### Example III: logistic loss

objective <- function(X, y, beta){
  linpred <- drop(X %*% beta)
  -sum(y * linpred) + sum(log(1 + exp(linpred)))
}
...
### Example from Paper 'Doubly regularized SVM' paper

muplus <- c(rep(1, 5), rep(0, 25))
muminus <- c(rep(-1,5), rep(0, 25))

coherence <- 0.8
Sigma <- matrix(nrow = 30, ncol = 30, data = 0)
Sigma[1:5, 1:5] <- coherence
diag(Sigma) <- 1

weightvector <- solve(Sigma, muplus - muminus)

library(mvtnorm)
set.seed(1811)
Xplus <- rmvnorm(n=100, mean = muplus, sigma = Sigma, method="chol")
set.seed(1812)
Xminus <- rmvnorm(n=100, mean = muminus, sigma = Sigma, method="chol")

X <- rbind(Xplus, Xminus)
y <- c(rep(1, 100), rep(0, 100))

glmord <- glm(y ~ X, family = "binomial")

### various variant of constrained logistic regression

check4 <- nnreginteriorpoint(X = X, y = y, objective = objective, gradient = gradient, Hessian = Hessian,
                               onlypositive = TRUE,
                               regamount = 0,
                               solutionpath = FALSE,
                               offset = FALSE,
                               beta0 = rep(0.01, ncol(X)),
                               lambda0 = NULL, 
                               eps = 1e-6,
                               cholmod = FALSE,
                               solutionpatharguments = list(regmax = NULL, gridlength = 100, fac = 0.001, verbose = TRUE, updatestart = TRUE),
                               trace = FALSE)
