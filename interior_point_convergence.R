...
nnreginteriorpoint <- function(X, y, objective, gradient, Hessian, onlypositive = TRUE,
  ...
  if(!offset){
    ...
    while(!convergence){
      ...
      ### line search 
      finished <- FALSE
      stepsize <- (-lambda / delta.lambda) ### compute maximum 
      ...
      while(!finished){
        ...
        #print("Finished loop")
        #print(betatilde)
        #print(lambda + stepsize*delta.lambda)
...
Hessian <- function(X, y, beta){
  ...
  #print(dim(Dinv^2))
  #print(dim(t(X)))
  #print(dim(Dinv^2 * as.vector(y)))
  t(X) %*% (Dinv^2 * as.vector(y)) %*% X 
}
...
xdal = daldata[, (names(daldata) %in% c("near_population", "near_employment"))]#"near_pop_old", "near_pop_child", "near_employment", "X15net_employment", "X15net_population", "near_business", "near_hospitality", "X30net_population"))]
ydal = daldata[, "ridership"]

xden = dendata[, (names(dendata) %in% c("near_population", "near_employment"))]#"near_pop_old", "near_pop_child", "near_employment", "X15net_employment", "X15net_population", "near_business", "near_hospitality", "X30net_population"))]
